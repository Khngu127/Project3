...
class MaintenanceRequest(db.Model):
    ...
    def __init__(self, apartment_number, area, description, photo=None):
        self.id = str(uuid.uuid4())[:8]  # Generate a random request ID
...
class Tenant(db.Model):
    ...
    def __init__(self, name, phone_number, email, check_in_date, check_out_date, apartment_number):
        self.id = str(uuid.uuid4())[:8]  # Generate a random tenant ID
        self.name = name
...
class StaffMember(db.Model):
    ...
    def __init__(self, name):
        self.id = str(uuid.uuid4())[:8]  # Generate a random staff ID
        self.name = name
...
class Manager(db.Model):
    ...
    def __init__(self, name):
        self.id = str(uuid.uuid4())[:8]  # Generate a random manager ID
        self.name = name
...
class MaintenanceSystem:
    ...
    def update_request_status(self, request_id, new_status, comment):
        ...
        if maintenance_request:
            ...
            maintenance_request.comment = comment  # Set the comment field
            db.session.commit()
            return True
        return False
    ...
    def add_tenant(self, name, phone_number, email, apartment_number):
        ...
        check_out_date = (datetime.now() + timedelta(days=365)).strftime(
            '%Y-%m-%d %H:%M:%S')  # Assuming a one-year lease
...
def member_interface():
    # Retrieve apartment_number from request parameters
    apartment_number = request.args.get('apartment_number')

    # Retrieve maintenance requests for the given apartment_number
...
def maintenance_staff_interface():
    # Assuming 'sort_by' is the query parameter for sorting
    sort_by = request.args.get('sort_by', 'request_id')  # Default sorting by request_id
    ...
    if request.method == 'POST':
        # Handle form submission for filtering maintenance requests
        ...
        # Display all maintenance requests initially
        requests = maintenance_system.browse_requests()

    # Sort the requests based on the 'sort_by' parameter
...
def update_status():
    if request.method == 'POST':
        ...
        # Assuming 'completed' is the new status
        new_status = 'completed'

        success = maintenance_system.update_request_status(request_id, new_status, comment)

        if success:
            # Optionally, you can save the comment in the database or perform other actions.
...
def management_interface():
    if request.method == 'POST':
        # Handle form submissions for adding, moving, or deleting a tenant
    ...
    # Sort the tenants based on the selected column and order
    if sort_by == 'id':
...
def login():
    ...
    if request.method == 'POST':
        ...
        if user:
            role = user.role
            apartment_number = user.apartment_number  # Retrieve apartment_number
...
def submit_request():
    ...
    # Fetch the latest requests after submission
    requests = MaintenanceRequest.query.filter_by(apartment_number=apartment_number).all()
